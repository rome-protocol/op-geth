...
func (miner *Miner) generateWork(params *generateParams, witness bool) *newPayloadResult {
	...
	for idx, tx := range params.txs {
		from, _ := types.Sender(work.signer, tx)
		work.state.SetTxContext(tx.Hash(), work.tcount)
		err = miner.commitTransaction(work, tx, idx, params.gasUsed[idx])
		if err != nil {
...
func (miner *Miner) commitBlobTransaction(env *environment, tx *types.Transaction) error {
	...
	receipt, err := miner.applyTransaction(env, tx, 0, 0)
	if err != nil {
		return err
	}
...
func (miner *Miner) applyTransaction(env *environment, tx *types.Transaction, index int, romeGasUsed uint64) (*types.Receipt, error) {
	...
	receipt, err := core.ApplyTransactionExtended(miner.chainConfig, miner.chain, &env.coinbase, env.gasPool, env.state, env.header, tx, &env.header.GasUsed, vm.Config{}, extraOpts, romeGasUsed)
	if err != nil {
