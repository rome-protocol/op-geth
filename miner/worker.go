...
func (w *worker) commitTransaction(env *environment, tx *types.Transaction, index int, romeGasUsed uint64, romeGasPrice uint64) ([]*types.Log, error) {
	if tx.Type() == types.BlobTxType {
		return w.commitBlobTransaction(env, tx)
	}
	receipt, err := w.applyTransaction(env, tx, index, romeGasUsed, romeGasPrice)
	if err != nil {
		return nil, err
	}
...
func (w *worker) applyTransaction(env *environment, tx *types.Transaction, index int, romeGasUsed uint64, romeGasPrice uint64) (*types.Receipt, error) {
	var (
		snap = env.state.Snapshot()
		gp   = env.gasPool.Gas()
	)

	receipt, err := core.ApplyTransaction(w.chainConfig, w.chain, &env.coinbase, env.gasPool, env.state, env.header, tx, &env.header.GasUsed, *w.chain.GetVMConfig(), romeGasUsed, romeGasPrice)
...
func (w *worker) generateWork(genParams *generateParams) *newPayloadResult {
	...
	for idx, tx := range genParams.txs {
		...
		_, err := w.commitTransaction(work, tx, idx, genParams.gasUsed[idx], genParams.gasPrice[idx])
		if err != nil {
