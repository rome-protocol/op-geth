...
func (w *worker) makeEnv(parent *types.Header, header *types.Header, genParams *generateParams) (*environment, error) {
	...
	log.Info("makeEnv: Solana metadata", "blockNumbers", len(genParams.solanaBlockNumbers), "timestamps", len(genParams.solanaTimestamps))
...
type generateParams struct {
	...
	solanaBlockNumbers []*uint64         // Optional Solana block numbers for each transaction
	solanaTimestamps   []*uint64          // Optional Solana timestamps for each transaction
...
func (w *worker) generateWork(genParams *generateParams) *newPayloadResult {
	...
	log.Info("generateWork: processing transactions", "txCount", len(genParams.txs), "solanaBlockNumbers", len(genParams.solanaBlockNumbers), "solanaTimestamps", len(genParams.solanaTimestamps))
