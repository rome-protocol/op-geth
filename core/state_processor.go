...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config, romeGasUsed []uint64, romeGasPrice []uint64, footPrints []string) (types.Receipts, []*types.Log, uint64, error) {
	...
	if metaSlot, metaHash, ok := p.bc.GetSolanaMetadata(blockHash); ok {
		log.Debug("StateProcessor.Process: retrieved Solana metadata", "blockHash", blockHash.Hex(), "slot", metaSlot, "solanaHash", metaHash.Hex(), "blockNumber", blockNumber.Uint64())
		solanaBlockNumber = &metaSlot
		solanaBlockHash = &metaHash
	} else {
		log.Debug("StateProcessor.Process: Solana metadata not found", "blockHash", blockHash.Hex(), "blockNumber", blockNumber.Uint64())
	}
