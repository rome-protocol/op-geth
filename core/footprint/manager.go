...
// isValidFootprint validates that a footprint string is safe to store.
// It only checks for maximum length to prevent DoS attacks via arbitrarily large payloads.
// Any hex string (including empty, "0x0", or full hashes) is allowed as long as it's not too large.
func isValidFootprint(footprint string) bool {
	// Maximum reasonable length: "0x" prefix + 64 hex chars = 66 characters
	// Allow some buffer for edge cases, but prevent DoS via huge strings
	const maxLength = 100
	
	if len(footprint) > maxLength {
		return false
	}
