...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	...
	defer bc.chainmu.Unlock()
	return bc.insertChain(chain, true, make([]uint64, 0), make([]uint64, 0))
}
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool, romeGasUsed []uint64, romeGasPrice []uint64) (int, error) {
...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator) (int, error) {
	...
	for i := len(hashes) - 1; i >= 0; i-- {
		...
		if len(blocks) >= 2048 || memory > 64*1024*1024 {
			...
			if _, err := bc.insertChain(blocks, true, make([]uint64, 0), make([]uint64, 0)); err != nil {
				return 0, err
			}
	...
	if len(blocks) > 0 {
		...
		return bc.insertChain(blocks, true, make([]uint64, 0), make([]uint64, 0))
	}
	return 0, nil
}
...
func (bc *BlockChain) recoverAncestors(block *types.Block) (common.Hash, error) {
	...
	for i := len(hashes) - 1; i >= 0; i-- {
		...
		if _, err := bc.insertChain(types.Blocks{b}, false, make([]uint64, 0), make([]uint64, 0)); err != nil {
...
// procedure.
func (bc *BlockChain) InsertBlockWithoutSetHead(block *types.Block, gasUsed []uint64, gasPrice []uint64) error {
	if !bc.chainmu.TryLock() {
		return errChainStopped
	}
	defer bc.chainmu.Unlock()

	_, err := bc.insertChain(types.Blocks{block}, false, gasUsed, gasPrice)
	return err
}
